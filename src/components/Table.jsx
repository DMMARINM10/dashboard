...
const Table = ({ route }) => {
    ...
    useEffect(() => {
        if (!page || !Number(page) || Number(page) > Number(totalPages) || Number(page) < 0) {
        ...
        if(!dataLoaded[page]) {
            ...
            .then((data) => {
                ...
                if(users) {
                    ...
                    .then((users) => {
                        ...
                        extraValues(data, perPage, setEmptyRows, emptyRows, element)
                    })
                } else {
                    dispatch(dispatchFunc(payload))
                    setData({
            ...
            if(users) {
                ...
                extraValues(dataLoaded[page], perPage, setEmptyRows, emptyRows, element)
            } else {
                setData({
                    users: [],
                    data: dataLoaded[page],
            ...
            <>
    {
        ...
        // !data.loading &&
            ...
            <Stack spacing={2}>
      <Pagination count={totalPages} page={Number(page)} shape="rounded" onChange={handlePage}/>
    </Stack>
        </>
    }
        </>
